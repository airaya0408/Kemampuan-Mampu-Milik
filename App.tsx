...
const App: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<{role: 'user' | 'agent', text: string}[]>([
    { role: 'agent', text: 'Selamat datang! Saya telah memproses keseluruhan 145 rekod data hartanah 2024. Ada apa-apa yang anda ingin tahu?' }
  ]);
  ...
  const handleSendDedicatedChat = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'agent', text: 'Maaf, ralat berlaku.' }]);
    } finally {
      setIsChatting(false);
    }
  };
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-slate-900 text-white p-8 h-full shrink-0">
        ...
        <div className="mt-auto pt-8 border-t border-slate-800">
          <div className="bg-slate-800/30 p-4 rounded-2xl border border-slate-700/50">
            <p className="text-[9px] font-bold text-slate-500 uppercase mb-2">Liputan Data 2024</p>
            <div className="w-full bg-slate-700 h-1.5 rounded-full overflow-hidden">
              <div className="bg-blue-500 h-full w-full"></div>
            </div>
            <p className="text-[10px] text-slate-400 mt-2 font-medium">145 Rekod Granular Dimuatkan</p>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-8 py-4 flex flex-col sm:flex-row justify-between items-center z-20 gap-4 shrink-0">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <select value={filterState} onChange={(e) => setFilterState(e.target.value)} className="bg-slate-100 border-none rounded-lg text-[10px] font-bold py-1.5 px-2.5 outline-none cursor-pointer">
                {states.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="bg-slate-100 border-none rounded-lg text-[10px] font-bold py-1.5 px-2.5 outline-none cursor-pointer">
                {houseTypes.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="bg-slate-100 border-none rounded-lg text-[10px] font-bold py-1.5 px-2.5 outline-none cursor-pointer">
                {affordabilityStatuses.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
          </div>
          <form onSubmit={handleSearch} className="relative w-full max-w-md">
            <input type="text" placeholder="Cari 145 rekod data..." className="w-full bg-slate-100 border-none rounded-full py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={query} onChange={(e) => setQuery(e.target.value)} />
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="max-w-7xl mx-auto space-y-12 pb-24">
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    ...
                    { label: 'Purata Indeks', val: `${averageMultiple}x`, desc: 'Nasional 2024' },
                    { label: 'Mampu Milik', val: filteredData.filter(d => d.medianMultiple <= 3).length, desc: 'Dari 145 Unit' },
                    ...
                    <div key={i} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                      ...
                      <p className="text-[10px] font-medium text-slate-500 mt-1 uppercase">{s.desc}</p>
                    </div>
                  ))}
                </div>

                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                  <h3 className="text-lg font-bold mb-10 italic">Jurang Kemampuan Median 2024 (Skala Nasional)</h3>
                  ...
                  <p className="text-[10px] text-center text-slate-400 mt-4 font-bold uppercase italic tracking-widest">Memaparkan 50 rekod pertama untuk visualisasi dashboard</p>
                </div>

                <div className="space-y-6">
                   <h3 className="text-xl font-black text-slate-800">Senarai Kad Hartanah ({filteredData.length} Keputusan)</h3>
            ...
            {activeTab === 'data-table' && (
              <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden">
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/80 sticky top-0 z-10">
                  <h3 className="text-xl font-black text-slate-800">Arkib Data Komprehensif (145 Rekod)</h3>
                  <div className="flex gap-2">
                    <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-lg">Aktif: {filteredData.length}</span>
                    <button onClick={() => window.print()} className="bg-white border border-slate-200 text-slate-700 px-4 py-1.5 rounded-full text-xs font-bold hover:bg-slate-50 transition-colors">Cetak Laporan</button>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 bg-slate-100 z-10 shadow-sm">
                      <tr className="text-[10px] uppercase font-black tracking-widest text-slate-500">
                        ...
                        <th className="px-8 py-4 border-b border-slate-200">Kategori</th>
                      </tr>
                    </thead>
                      ...
                      {filteredData.map((d, i) => (
                        <tr key={i} className="hover:bg-blue-50/30 transition-colors group">
                          ...
                          <td className="px-8 py-4">
                            <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase inline-block whitespace-nowrap ${
              ...
              <div className="max-w-4xl mx-auto bg-white p-12 rounded-[3rem] border border-slate-200 shadow-2xl min-h-[500px]">
                {loading ? (
                  <div className="py-24 flex flex-col items-center gap-6 text-center">
                    ...
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Menganalisis 145 Baris Data...</p>
                  </div>
                ) : (
            ...
            {activeTab === 'ai-agent' && (
              <div className="max-w-4xl mx-auto h-[600px] flex flex-col bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden">
                 <div className="bg-slate-900 p-8 flex items-center gap-4 shrink-0">
                    <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-bold">A</div>
                    <div>
                      <h3 className="text-white font-bold italic">Terminal Sembang Hartanah Nasional</h3>
                      <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Enjin Data 145 Rekod Aktif</p>
                    </div>
                 </div>
                 <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50">
                    {chatMessages.map((msg, idx) => (
                      <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] p-4 rounded-3xl ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none shadow-md' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-none shadow-sm'}`}>
                          <ReactMarkdown className="prose prose-sm max-w-none text-inherit">{msg.text}</ReactMarkdown>
                        </div>
                      </div>
                    ))}
                 </div>
                 <form onSubmit={handleSendDedicatedChat} className="p-6 bg-white border-t border-slate-100 flex gap-4 shrink-0">
                    <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Tanya tentang mana-mana 145 rekod data..." className="flex-1 bg-slate-100 border-none rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                    <button type="submit" disabled={isChatting} className="bg-blue-600 text-white p-4 rounded-2xl shadow-lg hover:bg-blue-700 transition-all disabled:opacity-50 active:scale-95"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg></button>
                 </form>
              </div>
            )}
          </div>
